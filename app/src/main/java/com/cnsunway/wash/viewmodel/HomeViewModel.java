...
import com.cnsunway.wash.resp.SearchResp;
...
public class HomeViewModel extends ViewModel{
    ...
    public static final int ORDER_STUTAS_ONE = 1;
    public static final int ORDER_STUTAS_TWO = 2;
    public static final int ORDER_STUTAS_THREE = 3;
    public static final int ORDER_STUTAS_FOUR = 4;
    public static final int ORDER_STUTAS_FIVE = 5;
    public static final int ORDER_STUTAS_SIX = 6;
    public static final int ORDER_STUTAS_SEVEN = 7;
    public static final int ORDER_STUTAS_EIGHT = 8;
    public static final int ORDER_STUTAS_NINE = 9;
    public static final int ORDER_STUTAS_TEN = 10;
    public static final int ORDER_STUTAS_ELEVEN = 11;
    public static final int ORDER_STUTAS_TWELVE = 12;
    /**
    ...
    public void setHomeLists(List<Order> homeLists) {
        List<Order> oldValue = this.homeLists;
    ...
    public void requestHomeData(Activity activity){
    ...
    public static String getOrderTimeForHome(Order order){
        int second = getOrderTimeSecond2(order);
    ...
    public static String getOrderTimeForHome2(Order order){
        if(TextUtils.isEmpty(order.getOrderTime())){
            return getOrderTimeForHome(order);
        }
        int second = msToSecond(Long.parseLong(order.getOrderTime()));
        int day = second/(3600*24);
        if(day >= 1){
            return day +"天" + (second%(3600*24))/3600 + "时";
        }else if(second > 3600){
...
//        second -= day*24*3600;
    ...
    public static String getOrderTimeForDetail3(String s,long timeInterval){
        int second = msToSecond(Long.parseLong(s));
        second += timeInterval;
//        int day = second/(3600*24);
//        second -= day*24*3600;
        ...
        return "" + hour + "时" + String.format("%02d",minute) + "分" + String.format("%02d",second) + "秒";
    }


    public static String getOrderTimeForDetail2(Order order){
        int second = getOrderTimeSecond2(order);
//        int day = second/(3600*24);
//        second -= day*24*3600;
        int hour = second/3600;
...
//    private static int getOrderTimeSecond2(Order order){
//        if(order == null){
//            return 0;
//        }
//        //
...
//        }
//
//        if(acceptDateStr == null && createDateStr != null){
//            Date createDate = toTime(createDateStr);
//            Date now = toServerTime(new Date());
//            ms = now.getTime()  - createDate.getTime();
//
...
//            Date acceptDate = toTime(acceptDateStr);
////            Log.e("expected time","now:" + expectedTime.getTime());
...
//    }
    ...
    private static int getOrderTimeSecond2(Order order){
        ...
        if(acceptDateStr == null && createDateStr != null){
            if(order.getStatus() < Order.STATUS_ARRIVED){
                Date createDate = toTime(createDateStr);
                Date now = toServerTime(new Date());
                ms = now.getTime()  - createDate.getTime();
            }else {
                Date createDate = toTime(createDateStr);
                ms = createDate.getTime();
            }
...
//            Log.e("expected time","now:" + expectedTime.getTime());
        ...
        if(expectedTime == null && pickedTme == null){
            if(order.getStatus() < Order.STATUS_ARRIVED){
                ...
                onDoorTime = toTime(order.getCreatedDate());
            }
...
//    private static int getOrderTimeSecend2(Order order){
//        if(order == null){
//            return 0;
//        }
//        if(TextUtils.isEmpty(order.getOrderTime())){
//            return getOrderTimeSecond(order);
//        }else {
//            return msToSecond(Long.parseLong(order.getOrderTime()))
//        }
//    }
    /**
     * 订单用时计算
...
//                }
    ...
    public static int getOrderSimpleStatus(Order order){
        switch(order.getStatus()){
            case Order.STATUS_NEW:
            case Order.STATUS_ALLOC:
            case Order.STATUS_TO_ASSIGN:
                return ORDER_STUTAS_ONE;
            case Order.STATUS_ACCEPTED:
                Date now = toServerTime(new Date());
                Date expectedTime = toTime(order.getExpectDateB());
                if(expectedTime != null && now.getTime() > expectedTime.getTime()){
            ...
            case Order.STATUS_TO_PICK:
                ...
                if(order.getPayStatus() == 10 ||
                        order.getPayStatus() == 20){
                    return ORDER_STUTAS_FOUR;
                }
                if(order.getPayStatus() == 30){
                    return ORDER_STUTAS_FIVE;
                }
                return ORDER_STUTAS_THREE;
            case Order.STATUS_PICKED:
                return ORDER_STUTAS_SIX;
            case Order.STATUS_IN_STORE:
                return ORDER_STUTAS_SEVEN;
            case Order.STATUS_WASHING:
                return ORDER_STUTAS_EIGHT;
            case Order.STATUS_WAIT_OUT:
                //TODO：虚状态
            {
                OrderCircle status = getVirtualOrderStatus(order);
                if(status == OrderCircle.CIRCLE_DISINFECTING){
                    return ORDER_STUTAS_NINE;
                }else if(status == OrderCircle.CIRCLE_CARING){
                    return ORDER_STUTAS_NINE;
                }else if(status == OrderCircle.CIRCLE_QUALIFYING){
                    return ORDER_STUTAS_NINE;
            ...
            case Order.STATUS_TO_CONFIRM:
                return ORDER_STUTAS_NINE;
            case Order.STATUS_WAY_BACK:
                return ORDER_STUTAS_TEN;
            case Order.STATUS_ARRIVED:
                return ORDER_STUTAS_ELEVEN;
            case Order.STATUS_DONE:
                return ORDER_STUTAS_TWELVE;
            case Order.STATUS_CANCEL:
                return -1;
        }
        return -1;
...
//            }
    ...
    public void onLoadMore() {
        ...
        LocationForService locationForService = UserInfosPref.getInstance(activity).getLocationServer();
        JsonVolley orderDoneVolley = new JsonVolley(activity, Const.Message.MSG_ORDER_DONE_SUCC, Const.Message.MSG_ORDER_DONE_FAIL);
        orderDoneVolley.addParams("pageSize",5);
        orderDoneVolley.addParams("pageNo",page);
        orderDoneVolley.requestGet(Const.Request.ordersearch,
                getHandler(), userInfos.getUser() == null ? "" : userInfos.getUser().getToken(),locationForService.getCityCode(),locationForService.getProvince(),locationForService.getAdcode(),locationForService.getDistrict()
        );
    }
