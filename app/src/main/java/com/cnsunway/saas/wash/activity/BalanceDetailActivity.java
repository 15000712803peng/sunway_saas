...
public class BalanceDetailActivity extends LoadingActivity implements XListView.IXListViewListener {
    XListView banlanceList;
    TextView titleText;
    List<BalanceItemParent> detailItems = new ArrayList<>();
    JsonVolley getBanlanceDetailVolley;
    BalanceDetailAdapter balanceAdapter;
    ...
    protected void initViews() {
        ...
        banlanceList.setPullLoadEnable(true);
        banlanceList.setPullRefreshEnable(false);
        titleText = (TextView) findViewById(R.id.text_title);
        titleText.setText(R.string.balance_detail);
        ...
        getBanlanceDetailVolley.requestGet(Const.Request.balancelog, getHandler(), UserInfosPref.getInstance(this).getUser().getToken(),locationForService.getCityCode(),locationForService.getProvince(),locationForService.getAdcode(),locationForService.getDistrict()
        );
        showCenterLoading();
        ...
        switch (msg.what) {
            case Const.Message.MSG_GET_BANLANCE_LOG_SUCC:
                ...
                if (msg.arg1 == Const.Request.REQUEST_SUCC) {
                    ...
                    GetBanlanceLogResp resp = (GetBanlanceLogResp) JsonParser.jsonToObject(msg.obj + "", GetBanlanceLogResp.class);
                    PagerBanlanceDetail pagerBanlanceDetail = resp.getData();
                    if (pagerBanlanceDetail != null && pagerBanlanceDetail.getDailyLogs() != null && pagerBanlanceDetail.getDailyLogs().size() > 0) {
                        totalDays = pagerBanlanceDetail.getTotalDays();
                        List<DayBalanceDetail> balanceDetails = pagerBanlanceDetail.getDailyLogs();
                        banlanceList.setVisibility(View.VISIBLE);
                        start += dayRanges;
                        addPagerBalanceDataToAdapter(balanceDetails);

                    } else {
                        banlanceList.stopLoadMore();
                        showNoData(R.string.no_banlance_detail);
                    }
    ...
    private void addPagerBalanceDataToAdapter(List<DayBalanceDetail> items) {
        for (DayBalanceDetail item : items) {
            detailItems.add(new BanlanceItemCategory(item.getDate()));
            detailItems.addAll(item.getLogs());
        }
        if (balanceAdapter == null) {
            balanceAdapter = new BalanceDetailAdapter(detailItems);
            banlanceList.setAdapter(balanceAdapter);
            banlanceList.setXListViewListener(this);
        } else {
            balanceAdapter.setItems(detailItems);
