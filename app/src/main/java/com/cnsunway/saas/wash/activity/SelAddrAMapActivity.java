            ...
            public void onClick(View view) {
                Intent intent = new Intent(SelAddrAMapActivity.this,SearchAddrActivity.class);
                 if(currentLocationAddr != null){
                     intent.putExtra("my_location",JsonParser.objectToJsonStr(currentLocationAddr));
                 }
...
//                            firstItem = items.get(0);
                            if(currentLocationAddr == null){
                                ...
                                currentLocationAddr.setLatitude(items.get(0).getLatLonPoint().getLatitude()+"");
                                currentLocationAddr.setLongtitude(items.get(0).getLatLonPoint().getLongitude()+"");
                                currentLocationAddr.setAddress(items.get(0).getTitle());
                                currentLocationAddr.setAddressDetail(items.get(0).getSnippet());
                            }
...
//                            myLocation.setSnap(items.get(0).getSnippet());
//                            myLocation.setTitle(items.get(0).getTitle());
//                        }
                    }
...
//                setPoiCenter();
        ...
        if(!TextUtils.isEmpty(addrStr)){
            addr = (Addr) JsonParser.jsonToObject(addrStr,Addr.class);
            fillCurrentLocation(addr);
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == REQUEST_SEARCH_ADDR && resultCode == RESULT_OK){
            if(data != null){
                ...
                String searchContent = data.getStringExtra("search_content");
                if(!TextUtils.isEmpty(searchContent)){
                    searchContentText.setVisibility(View.VISIBLE);
                    searchContentText.setText(searchContent);
                    editAddrText.setVisibility(View.INVISIBLE);
                }
                if(item != null){
                    needGeoSearch = false;
                    fillCurrentLocation(item);
